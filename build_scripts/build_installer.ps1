...
Write-Host "ShareJadPi v4.5.0 - Installer Builder" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Check if .exe exists
$exePath = "dist\ShareJadPi-4.5.0.exe"
if (-not (Test-Path $exePath)) {
    Write-Host "ERROR: ShareJadPi-4.1.2.exe not found!" -ForegroundColor Red
    Write-Host ""
...
Write-Host "[OK] ShareJadPi-4.5.0.exe found" -ForegroundColor Green
Write-Host ""
...
# Build the installer
$scriptPath = Join-Path $PSScriptRoot "build_tools\ShareJadPi-Installer-4.5.0.iss"
...
if ($LASTEXITCODE -eq 0) {
    ...
    $installerPath = "installer_output\ShareJadPi-4.5.0-Setup.exe"
    ...
    Write-Host "  [+] ShareJadPi-4.5.0.exe (auto-starts server for open-online)" -ForegroundColor White
